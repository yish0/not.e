...
import { ShortcutConfig, GlobalShortcutManager, LocalShortcutManager, ActionExecutor, ShortcutActionHandler } from './types'
...
export class ShortcutManager {
  ...
  async registerGlobalShortcutsFromConfig(): Promise<void> {
    ...
    console.log(`Registered ${results.filter(r => r.success).length}/${results.length} global shortcuts`)
  }
  ...
  async registerLocalShortcutsFromConfig(window: BrowserWindow): Promise<void> {
    ...
    console.log(`Registered ${results.filter(r => r.success).length}/${results.length} local shortcuts for window ${window.id}`)
  }

  // 개별 전역 단축키 등록 (설정에도 저장)
  async registerGlobalShortcut(key: string, action: string, description: string, category: string = 'custom'): Promise<boolean> {
    await this.ensureInitialized()
  ...
  async registerLocalShortcut(window: BrowserWindow, key: string, action: string, description: string, category: string = 'custom'): Promise<boolean> {
    await this.ensureInitialized()
  ...
  // 액션 등록
  registerAction(name: string, handler: ShortcutActionHandler, description: string, category: string): void {
