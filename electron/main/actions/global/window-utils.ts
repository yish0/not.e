...
export function restoreWindowOnCurrentDisplay(window: BrowserWindow): void {
  ...
  const newX = Math.max(
    workArea.x,
    Math.min(workArea.x + workArea.width - windowBounds.width, windowBounds.x)
  )
  const newY = Math.max(
    workArea.y,
    Math.min(workArea.y + workArea.height - windowBounds.height, windowBounds.y)
  )
...
export function getSidebarBounds(
  ...
  const x = position === 'left' ? workArea.x : workArea.x + workArea.width - width
...
export function showWindowAsSidebar(
  ...
  const sidebarBounds = getSidebarBounds(currentDisplay, position, width)
...
export async function showWindow(window: BrowserWindow): Promise<void> {
  ...
  if (process.platform === 'darwin') {
    ...
    const centerX =
      ...
      Math.floor((currentDisplay.workArea.width - windowBounds.width) / 2)
    const centerY =
      currentDisplay.workArea.y +
      Math.floor((currentDisplay.workArea.height - windowBounds.height) / 2)
